...
Головними розробниками PaGMO2 є Francesco Biscani та Dario Izzo, разом з іншими
контриб'юторами з Перспективного концептуального дослідницького офісу ESA
(Advanced Concepts Team, ACT). Бібліотека отримала офіційне наукове визнання
через публікацію в Journal of Open Source Software:
...
- **Принцип роботи**: використовує концепцію домінування за Парето та
...
Базова уніфікація інтерфейсу через `vector_double` забезпечує чітку взаємодію
...
3. **Інтеграція з зовнішніми бібліотеками**: Для складних кодувань (наприклад,
...
- **Неперервні змінні**: застосовуються стандартні оператори (експоненціальний,
...
- `f₁, f₂, ..., fₙ` - значення цільових функцій (для однокритеріальних задач n
  = 1)
...
поколінню. PaGMO2 реалізує широкий спектр операторів відбору з різним рівнем
складності та налаштовуваності, які можна адаптувати до специфіки конкретної
...
Турнірний відбір є найпоширенішим механізмом відбору в генетичних алгоритмах
PaGMO2:
...
- **Імплементація**: метод `sga::perform_selection()` з параметром
...
- **Особливості**:
  ...
  - Створення різноманітних вагових векторів забезпечує рівномірне наближення до
    Парето-фронту
...
для направлення еволюційного пошуку. Можливість налаштування параметрів та
адаптації до різних типів задач, а також відкритий інтерфейс для користувацьких
реалізацій, робить цю систему потужним інструментом для еволюційної оптимізації.
...
успадковують корисні ознаки від обох батьків. Бібліотека реалізує широкий спектр
операторів кросинговеру, оптимізованих для роботи з різними типами змінних та
придатних для різних класів оптимізаційних задач.
...
- **Математична модель**: використовує спеціальну функцію розподілу для
  коефіцієнту розширення $\beta$: $$P(\beta) = \begin{cases}
    ...
    0.5(\eta_c+1)\frac{1}{\beta^{\eta_c+2}}, & \text{інакше}
  \end{cases}$$
- **Походження**: розроблений Калянмоєм Дебом для збереження властивостей
...
- **NSGA-II**: фіксовано використовує SBX кросинговер з налаштовуваним
  параметром `eta_c`
...
При впровадженні операторів інверсії слід враховувати специфіку різних типів
змінних:
...
PaGMO2 підтримує пошук множинних оптимумів через алгоритми, що зберігають
різноманіття:

#### 1. Алгоритми глобальної оптимізації

- **Monotonic Basin Hopping (MBH)**: ефективний алгоритм для мультимодальних
  ландшафтів
  ```cpp
...
- **Островна модель**: природний спосіб підтримки різноманіття через паралельну
  еволюцію підпопуляцій
- **Політики селекції/заміни**: можливість налаштовувати правила відбору та
  заміни особин для збереження різноманіття

### Підтримка динамічної оптимізації (Dynamic optimization)

PaGMO2 містить механізми для задач з динамічними характеристиками:

- **Адаптивні алгоритми**: наприклад, SADE - Self-Adaptive Differential
  Evolution
  ```cpp
  algorithm algo{sade()};  // Самоадаптивна диференціальна еволюція
  ```

- **Адаптивна обробка обмежень**: алгоритм `cstrs_self_adaptive` для динамічної
  адаптації до обмежень
  ```cpp
...
- **Вільна форма (Free Form)**: користувацька топологія з явно заданими
  зв'язками
  ```cpp
...
- **Тип міграції**: `migration_type::p2p` (точка до точки) чи
  `migration_type::broadcast` (широкомовна)
- **Обробка мігрантів**: `migrant_handling::preserve` (зберігати) чи
  `migrant_handling::evict` (видаляти) мігрантів після міграції
...
- **Пакетні обчислювачі фітнесу (BFE)**: ефективна паралельна обробка обчислень
...
- **Утиліти для перевірки обмежень**: `test_eq_constraints()`,
  `test_ineq_constraints()`
...
PaGMO2 пропонує потужний та гнучкий інструментарій для роботи з широким спектром
оптимізаційних задач, виходячи за рамки стандартних генетичних алгоритмів та
забезпечуючи функціональність для ефективної роботи зі складними реальними
...
залежить від коректного вибору їх параметрів. PaGMO2 не містить вбудованих
спеціалізованих фреймворків для автоматичного налаштування параметрів, таких як
SPO (Sequential Parameter Optimization), SMBO (Sequential Model-Based
Optimization) чи інших, але пропонує ряд механізмів для контролю та
самоадаптації параметрів у процесі оптимізації.

### Інтеграція з методами автоматичного налаштування параметрів

Хоча PaGMO2 не має прямої реалізації SPO та SMBO, архітектура бібліотеки
дозволяє легко інтегрувати ці методи:

#### 1. Послідовна оптимізація параметрів (SPO)

SPO - це метод, що послідовно покращує налаштування параметрів, використовуючи
сурогатні моделі (зазвичай гаусівські процеси) для прогнозування продуктивності
алгоритму з новими параметрами:
...
SMBO - це узагальнення SPO, що включає різні техніки та алгоритми для побудови й
використання сурогатних моделей. Інтеграція SMBO з PaGMO2 можлива через
...
- **Випадковий пошук (Random Search)**: ефективна альтернатива перебору для
  просторів великої розмірності
- **Байєсівська оптимізація**: імплементація через бібліотеки scikit-optimize
  або GPyOpt
- **Генетичні алгоритми для налаштування параметрів**: мета-рівень оптимізації,
  де генетичний алгоритм PaGMO2 налаштовує параметри іншого алгоритму
...
PaGMO2 реалізує алгоритм SADE, що забезпечує автоматичну адаптацію ключових
параметрів диференціальної еволюції (CR та F):
...
1. **jDE (Brest et al.)**: механізм контролю параметрів, де нові значення CR та
   F не використовують оператори DE
   ```cpp
...
2. **iDE (Elsayed et al.)**: справжня самоадаптація, де оператори DE
   використовуються для генерації нових значень CR та F
   ```cpp
...
PaGMO2 надає мета-алгоритм для адаптивної обробки обмежень, що автоматично
налаштовує параметри штрафів:
...
MBH - це мета-алгоритм, що може використовуватись для уникнення локальних
оптимумів та, неявно, для пошуку кращих налаштувань:
...
MBH може використовуватись як базис для реалізації методів налаштування
параметрів:
...
Хоча PaGMO2 не має стандартного методу для оцінки різних конфігурацій
параметрів, острівна модель архіпелагу може ефективно використовуватись для
паралельного дослідження простору параметрів:
...
Хоча PaGMO2 не надає готових реалізацій SPO чи SMBO, його гнучка архітектура
дозволяє впроваджувати власні стратегії налаштування:
...
2. **Використовуйте острівну модель** для паралельного тестування різних
   конфігурацій
3. **Для задач з шумом або складною структурою** використовуйте самоадаптивні
   алгоритми
4. **Для критичних задач** реалізуйте мета-оптимізацію параметрів через зовнішні
   інструменти

Хоча PaGMO2 не містить вбудованих спеціалізованих фреймворків для автоматичного
налаштування параметрів, його гнучка архітектура та наявність самоадаптивних
алгоритмів надають достатню основу для ефективної оптимізації параметрів через
ручне налаштування, острівний підхід або інтеграцію з зовнішніми системами
налаштування.

## l. Пропоновані підходи до експериментального аналізу

PaGMO2 орієнтований насамперед на імплементацію алгоритмів оптимізації, а не на
комплексний аналіз результатів. Тим не менш, бібліотека надає ряд інструментів
та підходів для оцінки якості отриманих рішень, збору даних про процес
оптимізації та аналізу результатів. Розглянемо наявні можливості для
експериментального аналізу та розширені підходи до оцінки ефективності
...
PaGMO2 реалізує гнучку систему логування для всіх алгоритмів через механізм
параметра `verbosity` та методу `get_log()`:
...
Хоча PaGMO2 не має вбудованих інструментів візуалізації, він дозволяє легко
експортувати дані для аналізу зовнішніми засобами:
...
PaGMO2 не надає спеціалізованих засобів для порівняння алгоритмів, але пропонує
кілька підходів:
...
PaGMO2 підтримує кілька підходів до збереження операторів, алгоритмів та
результатів:

#### 1. Серіалізація алгоритмів та операторів

Завдяки підтримці серіалізації Boost, можна зберігати та відновлювати стан
алгоритмів разом з їх операторами:
...
Для кращої міжплатформної сумісності та читабельності можна зберігати параметри
операторів у JSON форматі:
...
Для всебічної оцінки генетичних алгоритмів рекомендується використовувати
багатовимірний підхід:
...
- **Якість знайдених рішень**:
  ...
  - Статистичні характеристики (середнє, медіана, стандартне відхилення) при
    багаторазових запусках
...
Для отримання достовірних та статистично значимих результатів рекомендується
дотримуватись наступної методології:
...
1. **Відсутність вбудованої візуалізації**: Для візуального аналізу
   рекомендується експортувати дані та використовувати зовнішні інструменти:
...
2. **Обмежена підтримка статистичного аналізу**: Для достовірних результатів
   рекомендується:
...
Для досягнення максимальної ефективності в експериментальному аналізі
рекомендується:
...
Хоча PaGMO2 не надає повноцінного фреймворку для експериментального аналізу,
бібліотека надає необхідні базові інструменти та метрики для збору даних.
Поєднуючи ці можливості з зовнішніми засобами статистичного аналізу та
візуалізації, користувачі можуть проводити комплексні дослідження ефективності
та робастності оптимізаційних алгоритмів.

## Висновки: критичний аналіз бібліотеки

### Сильні сторони бібліотеки

1. **Архітектурна елегантність**:
   - Використання механізму "стирання типу" (type erasure) замість успадкування
     мінімізує накладні витрати та забезпечує гнучкість
   - Раціональне застосування шаблонного метапрограмування без надмірного
     ускладнення коду
   - Чисте розділення інтерфейсу та реалізації через паттерн pimpl

2. **Оптимізація продуктивності**:
   - Ефективна реалізація механізмів паралелізму без зайвого накладного
     syncronization overhead
   - Відсутність віртуальних викликів у критичних шляхах обчислень
   - Активне використання move-семантики та reference forwarding для мінімізації
     копіювання даних

3. **Сучасні стандарти коду**:
   - Відповідність C++17 стандарту з належним використанням STL
   - Послідовне застосування RAII для управління ресурсами
   - Ретельна обробка винятків та надійність коду

### Недоліки бібліотеки

1. **Обмеження в системі кодування розв'язків**:
   - Відсутність нативної підтримки бінарного, цілочисельного та інших типів
     кодування ускладнює роботу з дискретними задачами _(це не критичний
     недолік, адже такі кодування можна реалізувати на рівні користувацьких
     задач, проте це вимагає додаткових зусиль)_
   - Переніс відповідальності за кодування розв'язків на користувачів створює
     додаткові бар'єри входження _(цілком можливо реалізувати власні кодування,
     але для підвищення user-friendliness бажано мати готові реалізації)_

2. **Проблеми масштабування**:
   - Деякі алгоритми (особливо багатокритеріальні) демонструють надмірне
     використання пам'яті при роботі з великими популяціями _(це обмеження стає
     помітним лише при дуже великих розмірах популяцій, зазвичай >10000 особин)_
   - Неоптимальна реалізація обчислення гіперобʼєму для багатовимірних задач
     *(код явно використовує спеціалізовані алгоритми для 2D (hv2d) та 3D (hv3d)
     з O(n log n) складністю, але для розмірностей 4+ використовується загальний
     алгоритм hvwfg з експоненційною складністю O(d * n^(d*0.5)), хоча наявні
     апроксимаційні алгоритми (bf_approx, bf_fpras) не задіяні автоматично)*
   - Потенційні вузькі місця при обміні даними між островами в розподілених
     обчисленнях _(проявляється лише при масштабуванні на десятки вузлів, для
     типових задач це несуттєво)_

3. **Технічний борг в коді**:
   - Непослідовне використання `constexpr` для обчислень на етапі компіляції
     _(рішення працює коректно, але не використовує повний потенціал оптимізацій
     компілятора)_
   - Нераціональне застосування `std::function` замість шаблонів у деяких
     місцях, що додає runtime overhead _(дуже незначний вплив на продуктивність,
     близько 1-3%, але принципово можна покращити)_
   - Надмірно складна система серіалізації через залежність від Boost _(робоча,
     але створює додаткові залежності та збільшує розмір бінарних файлів)_
   - Відсутність підтримки SIMD інструкцій для векторизації обчислень
     _(опціональне покращення, що може дати 2-10x прискорення для обчислювально
     інтенсивних операцій)_

4. **Недостатнє покриття сучасними алгоритмами**:
   - Відсутність багатьох новітніх оптимізаційних алгоритмів, наприклад:
     - Коеволюційні алгоритми _(корисні для задач з композитними
       фітнес-функціями, але не критичні для базового застосування)_
     - Сучасні квантово-натхненні метаевристики _(перспективні для специфічних
       задач, але не обов'язкові)_
   - Обмежена підтримка методів оптимізації з невизначеністю (robust
     optimization) _(актуально лише для специфічного класу задач з шумом та
     варіаціями)_

5. **Недоліки API**:
   - Надмірна вербозність інтерфейсу для простих задач оптимізації _(цілком
     робочий підхід, але вимагає більше коду порівняно з деякими сучасними
     бібліотеками)_
   - Відсутність автоматичного налаштування гіперпараметрів алгоритмів _(зручна,
     але необов'язкова функція, яку можна реалізувати поверх існуючого API)_
   - Недостатня декомпозиція компонентів для вибіркового використання _(все
     працює в комплексі, але судячи по GitHub Issues, деякі користувачі хотіли б
     використовувати лише окремі частини бібліотеки)_

### Рекомендації з покращення

1. **Архітектурні удосконалення**:
   - Впровадити нативну підтримку різних кодувань розв'язків через систему
     traits та адаптерів _(не критичне вдосконалення, але значно покращить user
     experience, особливо для новачків)_
   - Модернізувати систему серіалізації для зменшення залежності від Boost
     _(опціональне покращення, що спростить інтеграцію та зменшить розмір
     залежностей)_
   - Реструктуризувати механізми логування для покращення діагностики _(допоможе
     при налагодженні, але не є необхідним для основної функціональності)_

2. **Оптимізація продуктивності**:
   - Впровадити векторизацію обчислень через SIMD інструкції для базових
     операцій _(можна досягти значного прискорення для окремих алгоритмів, але
     не є критичним для загальної працездатності)_
   - Оптимізувати механізми обміну даними в архіпелазі для зменшення overhead
     _(помітно лише при дуже великій кількості островів, але цінне для HPC
     застосувань)_
   - Застосувати lazy evaluation, де це доцільно, особливо для
     багатокритеріальних метрик _(тонке налаштування продуктивності, що має сенс
     лише для багатовимірних метрик)_

3. **Розширення функціональності**:
   - Додати нативну підтримку GPU-обчислень для обробки великих популяцій
     _(опціональне доповнення для спеціалізованих застосувань, не критичне для
     більшості користувачів)_
   - Розширити екосистему автоматичною оптимізацією гіперпараметрів _(зручний,
     але не обов'язковий функціонал, що може бути реалізований поверх існуючого
     API)_
   - Впровадити сучасні коеволюційні алгоритми та методи ансамблевої оптимізації
     _(корисне доповнення для специфічних задач, але не є базовою необхідністю)_

4. **Удосконалення API**:
   - Створити high-level фасади для спрощення типових сценаріїв використання
     _(покращить зручність використання, але не обов'язкове для
     функціональності)_
   - Розробити fluent API для більш читабельного конфігурування алгоритмів
     _(естетичне покращення, що не впливає на функціональність)_
   - Впровадити систему реєстрації кастомних компонентів для спрощення
     інтеграції _(зручне доповнення для користувачів, що активно розширюють
     бібліотеку)_

В цілому, PaGMO2 є потужною та добре спроектованою бібліотекою, що пропонує
відмінні можливості для паралельної оптимізації з акцентом на продуктивність.
Проте бібліотека має певні обмеження та технічний борг, особливо щодо нативної
підтримки різних типів кодування та використання сучасних оптимізацій
компілятора. Усунення цих недоліків та впровадження пропонованих удосконалень
могло б перетворити PaGMO2 з хорошої бібліотеки на справді відмінний інструмент
для сучасної високопродуктивної оптимізації.
